...
## 7. Ручная проверка после деплоя

После деплоя мини-приложения рекомендуется пройти полный флоу оформления заказа и зафиксировать результат:

1. **Корзина → Оформить заказ**: открыть корзину с товарами, нажать «Оформить заказ».
2. **Форма**: заполнить имя, телефон, доставка/самовывоз, дата и время (или «Доставка по готовности»), получатель, комментарий (опц.), способ оплаты, при наличии бонусов — чекбокс «Использовать бонусы».
3. **Создание заказа**: нажать «Оформить заказ»; проверить, что заказ создаётся, отображаются итог и списанные бонусы (если использованы).
4. **Шаг оплаты**: отображаются реквизиты / инструкция по оплате, кнопка загрузки чека (если предусмотрено).
5. **Уведомления**: при настроенных уведомлениях — сообщение менеджеру в группу; при подтверждении/отклонении — сообщение клиенту.
6. **Отмена заказа**: на шаге оплаты нажать «Отменить заказ», подтвердить; проверить возврат бонусов и возврат к форме.

При обнаружении багов — зафиксировать и исправить отдельными правками.
